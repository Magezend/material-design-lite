...
test('default adapter returns a complete adapter implementation', t => {
  verifyDefaultAdapter(MDLSelectFoundation, [
    ...
    'deregisterInteractionHandler', 'focus', 'makeFocusable', 'makeUnfocusable',
    'getComputedStyleValue', 'setStyle', 'create2dRenderingContext', 'openMenuWithFocusedOptionAtIndex',
...
test('#setDisabled makes the select unfocusable when true', t => {
  const {foundation, mockAdapter} = setupTest();
  foundation.setDisabled(true);
  t.doesNotThrow(() => td.verify(mockAdapter.makeUnfocusable()));
  t.end();
});
...
test('#setDisabled makes the select focusable when false', t => {
  const {foundation, mockAdapter} = setupTest();
  foundation.setDisabled(false);
  t.doesNotThrow(() => td.verify(mockAdapter.makeFocusable()));
  t.end();
});
