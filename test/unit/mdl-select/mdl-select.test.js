...
test('adapter#makeFocusable sets the root element\'s tabindex to 0', t => {
  const {component, fixture} = setupTest();
  fixture.tabIndex = -1;
  component.getDefaultFoundation().adapter_.makeFocusable();
  t.equal(fixture.tabIndex, 0);
  t.end();
});

test('adapter#makeUnfocusable sets the root element\'s tabindex to -1', t => {
  const {component, fixture} = setupTest();
  fixture.tabIndex = 0;
  component.getDefaultFoundation().adapter_.makeUnfocusable();
...
test('adapter#openMenuWithFocusedOptionAtIndex shows the menu with the given focusIndex', t => {
  const {component, menu} = setupTest();
  component.getDefaultFoundation().adapter_.openMenuWithFocusedOptionAtIndex(1);
